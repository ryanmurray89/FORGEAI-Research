# FORGE-NOTE-016 — Telemetry-Backed User Profiles, Community Scoring, and Privacy Controls
...
FORGE added a complete user-profile and community analytics layer without creating a second telemetry system.
...
That keeps product analytics tied to the same persisted runtime measurements already used elsewhere in FORGE.
...
`Estimated active time with FORGE`
...
If FORGE grows substantially, the same public interface can later be backed by cached/materialized user statistics without changing the product contract.
