# FORGE-EXP-008 — Cross-Model Claim Verification and Human-Review Gating Pilot
...
Can FORGE independently check generated candidate claims against their frozen evidence using a different model family — while still failing closed on weak evidence and preserving separate human-review requirements for higher-risk material?
...
Across four recorded runs, FORGE performed **21 card-evaluation executions**.
...
FORGE did not infer that an incomplete verifier response meant the missing claim was safe.
...
Model-based evidence verification still inherits model limitations and therefore remains only one layer in the FORGE trust lifecycle.
